...
\subsection{Abtastung (Sampling)}
	...
	$$ \text{Idealer Sampler:} \qquad \delta_{T_S}(t) = \sum\limits_{n=-\infty}^{+\infty} \delta(t-nT_S) \; \laplace \; \omega_S \cdot \delta_{\omega_S} (\omega) = \omega_S \sum\limits_{n=-\infty}^{+\infty} \delta(\omega - n \omega_S) \qquad \text{mit  } \omega_S = \frac{2\pi}{T_S}$$
	\begin{center}
		\includegraphics[width=10cm]{bilder/dig_sampler_ideal.png}
	\end{center}

	\begin{equation*}
		\text{Idealer Abtaster:} \qquad m_s(t) = ...
	\end{equation*}	 
	\[\TransformVert\]
	\begin{equation*}
		M(\omega)
